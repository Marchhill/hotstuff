...
for (rate, n) in test_iter:
	...
	time.sleep(5)
	subprocess.run('eval $(opam env) OCAML_LANDMARKS=output=profile dune exec -- ./live_test.exe ' + str(n) + ' -t ' + str(experiment_time) + ' -r ' + str(rate) + ' --times "./graphs/data/times-' + name + '.csv" --stats "./graphs/data/dummystats.csv"', shell=True)
	time.sleep(1)
	kill_processes()
	processes = []