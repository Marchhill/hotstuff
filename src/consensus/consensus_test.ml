...
let deliver_command id nodes =
	let nodes, _ = advance_leader id nodes [ClientCmd {data = (Fmt.str "hello%d#%d" id (!sent)); callback_id = !sent}] in
	sent := !sent + 1;
	nodes
...
let%expect_test "leader can commit from later view" =
	...
	let new_view_events = [
		NewView {id = 0; view = 99; tcp_lens = []; msg_type = NewView; node = None; justify = (Some {msg_type = PrepareAck; view = 70; node = (create_leaf None (Cmd_set.singleton {data = "woo"; callback_id = 0})); signature = None; ids = []}); partial_signature = None};
		...
		NewView {id = 3; view = 99; tcp_lens = []; msg_type = NewView; node = None; justify = (Some {msg_type = PrepareAck; view = 67; node = (create_leaf None (Cmd_set.singleton {data = "woo"; callback_id = 0})); signature = None; ids = []}); partial_signature = None}
	] in
	let (s, a) = advance_multiple s new_view_events in
	let s = {s with s = {s.s with phase = PreCommit}} in
	List.iter print_action a;
