...
\begin{algorithm}[h!]
	...
	\Procedure{ON{\large R}ECEIVE{\large P}ROPOSAL}{$\text{MSG}\textsubscript{\textit{v}}(\text{GENERIC}, \textit{b\textsubscript{new}}, \textit{qc})$}
		\color{Green}
		\If {$ v = \text{GET{\large L}EADER}(\textit{m.view})$} %***check that they are leader of the view!!!
			\If {$ \textit{m.view} > \textit{curView} \land \textit{m.view} = \textit{qc.view} + 1$} %***check that they are leader of the view!!!
