...
To solve Threat \ref{threat2}, make replicas \textit{lock} on a proposal once they receive a \textit{commit} message, and not accept a pre-proposal for a conflicting log.
...
Unit testing was carried out using `expect tests', which compare a program trace to the correct output. A testing suite of expect tests verifies that the program behaves as specified in the HotStuff paper. This suite has 100\% code coverage of the consensus state machine code.%, the coverage report is at \textit{\_coverage/index.html}.
