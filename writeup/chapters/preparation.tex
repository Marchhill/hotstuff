...
In this section, we extend our non-byzantine algorithm (Section \ref{nonbyzconsensus}) to achieve consensus under a byzantine system model (assumption \ref{byzassumption}). To do this we must first introduce the quorum certificate (QC), a cryptographic proof that a leader has received a quorum of acks. We then consider the threats posed by byzantine nodes, give an algorithm that solves these problems, and make an argument for safety. Examples of similar algorithms include Tendermint \cite{kwonTendermintConsensusMining2014} and Casper \cite{buterinCasperFriendlyFinality2019}.
